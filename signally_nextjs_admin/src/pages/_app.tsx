...
export default function App(props: MyAppProps & { colorScheme: ColorScheme }) {
  ...
  const streamUser = useFirestoreStoreAdmin((state) => state.streamFirebaseUser);
  const { streamSignals, streamAnnouncements, streamAuthUsers } = useFirestoreStoreAdmin((state) => state);
